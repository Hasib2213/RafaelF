...
import { Mail, Eye, EyeOff, Sparkles, Headphones, CheckCircle2, ArrowRight, Check, X, KeyRound } from "lucide-react";
...
function LoginForm() {
  ...
  const [successMsg] = useState<string | null>(
    registered ? "Account created successfully! Please enter your password to log in." : null
  );
    ...
    <div className="relative min-h-screen w-full bg-[#0F172A] text-white flex items-center justify-center p-4 sm:p-6 lg:p-8 font-['Lato',sans-serif] overflow-x-hidden">
      ...
      <div className="relative z-10 w-full max-w-[1408px] min-h-[960px] flex flex-col lg:flex-row items-center justify-center gap-7 lg:gap-10">
        ...
        <motion.div
          ...
          className="hidden lg:flex relative w-[680px] xl:w-[720px] h-[880px] rounded-2xl overflow-hidden border border-[#383850]/60 bg-[#151628] shadow-2xl flex-col justify-between p-12 shrink-0 select-none"
        >
          ...
          <div className="relative z-10 flex flex-col gap-6">
            <div className="p-6 rounded-xl bg-white/[0.08] backdrop-blur-xl border border-white/15 shadow-xl flex flex-col gap-3">
              ...
              <p className="text-xl font-medium leading-snug text-white/95">
                &ldquo;Your daily intelligence feed is ready. Catch up on key insights from your followed channels.&rdquo;
              </p>
        ...
        >
          ...
          <div className="w-full text-center">
            ...
            <p className="text-[#D0D0D0] text-base mt-2">
              Enter your credentials to access your audio briefings
            </p>
          </div>
          ...
          <div className="w-full relative rounded-xl bg-[#151628] border border-[#383850] shadow-[0px_16px_50px_#0E011D] p-6 sm:p-8 flex flex-col gap-6 overflow-hidden">
            ...
            <div
              className="absolute pointer-events-none w-[220px] h-[50px] left-1/2 top-[50%] -translate-x-1/2 -translate-y-1/2 rounded-full opacity-60"
              style={{
            ...
            {successMsg && (
              <div className="p-3.5 rounded-lg bg-emerald-500/15 border border-emerald-500/30 text-emerald-300 text-sm text-center flex items-center justify-center gap-2">
            ...
            <form onSubmit={handleSubmit} className="relative z-10 flex flex-col gap-5">
              ...
              <div className="flex items-center justify-between py-1">
                ...
                <label className="flex items-center gap-2.5 cursor-pointer select-none group">
                  <div
                    ...
                    className={`w-5 h-5 rounded flex items-center justify-center border transition-all ${
                      rememberMe
                        ? "bg-[#7A3BED] border-[#7A3BED] text-white shadow-sm shadow-purple-900/40"
                        : "bg-[#27273F] border-[#383850] text-transparent hover:border-[#7A3BED]/60"
                    }`}
                  >
                    <Check className="w-3.5 h-3.5 stroke-[2.5]" />
                  </div>
                  <span className="text-base text-[#D0D0D0] group-hover:text-white transition-colors">
                    Remember Me
                  </span>
                </label>
                ...
                <Link
                  ...
                  className="text-[#FF5B5B] hover:text-red-400 text-base font-normal transition-colors cursor-pointer"
                >
                  Forget Password?
                </Link>
              </div>
      ...
      <AnimatePresence>
        {isForgotModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 figma-backdrop-overlay">
            <motion.div
              initial={{ opacity: 0, scale: 0.95, y: 10 }}
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.95, y: 10 }}
              className="relative w-full max-w-md p-6 sm:p-8 rounded-2xl bg-[#151628] border border-[#383850] shadow-2xl flex flex-col gap-6"
            >
              {/* Close button */}
              <button
                type="button"
                onClick={() => setIsForgotModalOpen(false)}
                className="absolute top-4 right-4 p-2 text-[#D0D0D0] hover:text-white transition-colors"
              >
                <X className="w-5 h-5" />
              </button>

              <div className="flex flex-col items-center text-center gap-3">
                <div className="w-12 h-12 rounded-xl bg-[#2563EB]/20 border border-[#2563EB]/30 flex items-center justify-center text-cyan-300">
                ...
                <form onSubmit={handleSendResetLink} className="flex flex-col gap-4">
                  <div className="flex flex-col gap-2">
                    ...
                    <input
                      ...
                      className="w-full h-11 px-4 rounded-lg bg-[#27273F] border border-[#383850] text-white text-sm outline-none focus:border-[#7A3BED]"
                    />
                  </div>
                  <button
                    type="submit"
                    className="w-full h-11 rounded-lg bg-gradient-to-r from-[#2563EB] to-[#7A3BED] text-white font-semibold text-sm hover:brightness-110 active:scale-[0.99] transition-all"
                  >
                    Send Recovery Link
                  </button>
                </form>
